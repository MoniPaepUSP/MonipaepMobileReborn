...
const atendimentoQuestions: FAQTopicQuestion[] = [
  ...
  },
  {
    ...
    answer: "Você precisará de um documento de identificação com foto e seu cartão do SUS."
  },
  {
    question: "Como posso agendar uma consulta?",
    answer: "Você pode agendar uma consulta pelo telefone ou diretamente no site da UPA."
  }
]
...
const principaisSintomasQuestions: FAQTopicQuestion[] = [
  {
    ...
    answer: "Os principais sintomas incluem febre alta, dor de cabeça intensa, dor atrás dos olhos, dores musculares e nas articulações, náuseas, vômitos e erupções cutâneas."
  },
  {
    question: "Como diferenciar dengue de uma gripe comum?",
    answer: "A dengue geralmente causa dores mais intensas e persistentes nas articulações e músculos, além de erupções cutâneas e febre alta que pode durar de 2 a 7 dias."
  },
  {
    question: "O que devo fazer se suspeitar que estou com dengue?",
    answer: "Procure um serviço de saúde imediatamente para avaliação e tratamento adequado."
  }
]
...
const cuidadosIniciaisQuestions: FAQTopicQuestion[] = [
  {
    ...
    answer: "Mantenha-se hidratado bebendo bastante líquido, evite medicamentos que contenham ácido acetilsalicílico e procure atendimento médico."
  },
  {
    question: "Posso tomar medicamentos para aliviar os sintomas?",
    answer: "Evite medicamentos como aspirina e ibuprofeno, que podem agravar a condição. Paracetamol pode ser utilizado para aliviar dores e febre, mas sempre sob orientação médica."
  },
  {
    question: "Como prevenir a disseminação da dengue em casa?",
    answer: "Elimine locais de água parada, use repelentes e mantenha portas e janelas fechadas ou com telas de proteção."
  }
]
...
const informacoesDengueQuestions: FAQTopicQuestion[] = [
  {
    ...
    answer: "A dengue é transmitida pela picada do mosquito Aedes aegypti infectado com o vírus da dengue."
  },
  {
    question: "Quais são os tipos de dengue?",
    answer: "Existem quatro sorotipos diferentes do vírus da dengue: DENV-1, DENV-2, DENV-3 e DENV-4."
  },
  {
    question: "Qual é o período de incubação do vírus da dengue?",
    answer: "O período de incubação varia de 4 a 10 dias após a picada do mosquito infectado."
  }
]
...
const suporteTecnicoQuestions: FAQTopicQuestion[] = [
  {
    ...
    answer: "O suporte técnico está disponível de segunda a sexta-feira, das 8h às 18h."
  },
  {
    question: "Como entrar em contato com o suporte técnico?",
    answer: "Você pode entrar em contato pelo telefone 0800-123-456 ou pelo e-mail suporte@exemplo.com."
  },
  {
...
export default function FrequentQuestionsComponent(): JSX.Element {
  ...
  const filteredFAQ = shouldFilter ? FAQ_QUESTIONS.filter((faq): boolean => {
    const { topic } = faq;
  ...
  return (
    <View style={styles.container}>
      ...
      {filteredFAQ.map((faq, index): JSX.Element => {
        ...
        return (
          <Accordion key={`${index}-${topic}`} title={topic} backgroundColor={colors.blue} color={colors.white} borderColor={colors.white}>
            {faq.questions.map((q, i): JSX.Element => {
              const { answer, question } = q;

              return (
                <Accordion key={`${i}-${question}`} title={question} subAccordion backgroundColor={colors.blue_light} color={colors.white} borderColor={colors.white}>
